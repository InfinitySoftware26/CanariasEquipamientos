...
import { StaffSociety, StaffSocietyStatus } from '../entities/staff-society.entity';
...
export class StaffSocietiesRepository implements IStaffSocietiesRepository {
  ...
  async upsert(staffId: string, societyId: string, status: StaffSocietyStatus): Promise<void> {
    const existing = await this.repo.findOne({ where: { staffId, societyId } });
    if (existing) {
      await this.repo.update({ staffSocietyId: existing.staffSocietyId }, { status });
    } else {
