...
export class RouteSheetsController {
  ...
  findAll(
    ...
    if (user.role === StaffRole.COLLECTOR) {
      return this.routeSheetsService.findByStaff(user.sub, user.societyId, filters);
    }
  ...
  @ApiOperation({ summary: 'Generar hoja de ruta (auto-completa cuotas y entregas pendientes)' })
