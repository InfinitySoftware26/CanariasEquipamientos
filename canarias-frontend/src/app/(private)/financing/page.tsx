"use client";

import { useAuthStore } from "@/store/auth.store";
import { StaffRole } from "@/types/auth.types";
import { useFinancingConfig } from "@/hooks/financing/useFinancingConfig";

import { FinancingConfigForm } from "@/components/financing/FinancingConfigForm";
import { LoanSimulator } from "@/components/financing/LoanSimulator";

const ALLOWED_ROLES = [StaffRole.ADMIN, StaffRole.MANAGER, StaffRole.SUPER_ADMIN];

export default function FinancingPage() {
  const activeRole = useAuthStore((state) => state.activeRole);
  const allowed = !!activeRole && ALLOWED_ROLES.includes(activeRole);

  const { config, loading, error, refresh, save, saving } =
    useFinancingConfig();

  if (!allowed) {
    return (
      <div className="rounded-2xl border border-white/10 bg-[#0E1726] p-8 text-center">
        <p className="text-white/60">
          No tenés permisos para acceder a esta sección.
        </p>
      </div>
    );
  }

  return (
    <div className="space-y-8">
      {/* HEADER */}
      <section>
        <h1 className="text-3xl font-bold text-white">Financiación</h1>

        <p className="mt-2 text-white/60">
          Configuración de tasas y simulador de cuotas.
        </p>
      </section>

      {loading && (
        <div className="space-y-4">
          <div className="h-64 animate-pulse rounded-3xl border border-white/10 bg-white/5" />

          <div className="h-64 animate-pulse rounded-3xl border border-white/10 bg-white/5" />
        </div>
      )}

      {!loading && error && (
        <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-8 text-center">
          <p className="text-red-300">{error}</p>

          <button
            onClick={refresh}
            className="mt-4 inline-flex items-center gap-2 rounded-xl border border-white/10 px-4 py-2 font-medium text-white transition hover:bg-white/5"
          >
            Reintentar
          </button>
        </div>
      )}

      {!loading && !error && config && (
        <div className="space-y-8">
          <FinancingConfigForm
            config={config}
            saving={saving}
            onSave={async (payload) => {
              await save(payload);
            }}
          />

          <LoanSimulator
            rates={{
              rate3: config.rate3,
              rate6: config.rate6,
              rate9: config.rate9,
            }}
          />
        </div>
      )}
    </div>
  );
}
